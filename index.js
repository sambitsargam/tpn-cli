...
async function main() {
  ...
  if (opts.validator) {
    ...
    const { axon } = await inquirer.prompt([{ type: 'list', name: 'axon', message: chalk.cyan('Select a TPN validator:'), choices: validators.map(v => ({ name: `UID ${v.UID} — ${v.Axon}`, value: v.Axon })) }]);
    API_BASE = axon;
  }
  ...
  const rawCodes = await axios.get(`http://${API_BASE}/api/config/countries`).then(r => r.data).catch(() => { spinner1.fail(chalk.red('Could not fetch regions')); process.exit(1); });
  spinner1.succeed(chalk.green(`Found ${rawCodes.length} regions`));

  // —— Determine country & lease ——
  ...
  if (useNonInteractive) {
    ...
    const countryAnswer = await inquirer.prompt([{ type: 'autocomplete', name: 'countryName', message: chalk.cyan('Select exit country:'), pageSize: 6, source: (_, input = '') => rawCodes.map(code => ({ name: regionNames.of(code), code })).filter(c => c.name.toLowerCase().includes(input.toLowerCase())).map(c => c.name) }]);
    countryName = countryAnswer.countryName;
    geo = rawCodes.find(c => regionNames.of(c) === countryName);
    const leaseAnswer = await inquirer.prompt([{ type: 'input', name: 'lease', message: chalk.cyan('Lease duration (minutes):'), default: '15', validate: v => (!isNaN(parseFloat(v)) && parseFloat(v) > 0) || 'Enter a positive number' }]);
    leaseMinutes = parseFloat(leaseAnswer.lease);
  }

  // —— Show IP before connecting ——
  ...
  const peerConfig = await axios.get(`http://${API_BASE}/api/config/new`, { params: { format: 'text', geo, lease_minutes: leaseMinutes } }).then(r => r.data).catch(() => { spinner2.fail(chalk.red('Lease failed')); process.exit(1); });
  ...
  // —— Write + Up ——
  const cfgPath = process.getuid && process.getuid() === 0 ? '/etc/wireguard/tpn.conf' : './tpn.conf';
  fs.writeFileSync(cfgPath, peerConfig);
  console.log(boxen(chalk.blue(`⚙ Bringing up WireGuard with TPN VPN (${countryName})`), { padding: 1, borderColor: 'blue', borderStyle: 'round' }));
  await execa('wg-quick', ['up', cfgPath], { stdio: 'ignore' });

  // —— Show IP after connecting ——
  ...
  process.on('SIGINT', async () => {
    ...
    if (shouldTearDown) {
      ...
      try { await execa('wg-quick', ['down', cfgPath], { stdio: 'ignore' }); } catch { }
      process.exit(0);
    } else {
  ...
  const bar = new cliProgress.SingleBar({ format: chalk.magenta('Lease 🔒 [{bar}] {percentage}% | {value}/{total}s'), hideCursor: true }, cliProgress.Presets.shades_classic);
